    ...
    <div className='flex flex-col gap-4 p-2'>
      ...
      <div className={`flex gap-10 items-start ${showFilter ? 'flex-col' : 'flex-row'}`}>
        <div className={`w-full sm:w-1/5 space-y-4 text-left font-medium items-start text-gray-600 ${showFilter ? 'block' : 'hidden'} sm:block`}>
          <p onClick={()=>speciality ==='General Physician' ? navigate('/doctors') : navigate('/doctors/Generalphysician')} className={`w-full sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${speciality === "General physician" ?  "bg-indigo-100 text-black1" : ""} `}>General Physician</p>
          <p onClick={()=>speciality ==='Gynocologist' ? navigate('/doctors') : navigate('/doctors/Gynocologist')} className={`w-full sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${speciality === "Gynocologist" ?  "bg-indigo-100 text-black1" : ""} `}>Gynocologist</p>
